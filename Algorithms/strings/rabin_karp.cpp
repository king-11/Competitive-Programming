...
void solve() {
  ...
  int B = 31; // lower upper both then p = 53 else 31;
  if(n < m) return;

  // pattern hash
  int hp = 0;
  for(int i = 0; i < m; i++){
    hp = modulo(hp * B + pattern[i], MOD);
  }
  
  // text starting hash
  int ht = 0;
  for(int i = 0; i < m; i++){
    ht = modulo(ht * B + text[i],MOD);
  }


  ostringstream s1;
  if(hp == ht && text.substr(0,m) == pattern){
    s1 << "match at " << 1 << endl;
  }
  ...
  for(int i = m; i < n; i++){
    ...
    // cout << ht << " ";
    if(ht == hp){
      for(int j = i - m + 1, k = 0; j <= i && k < m; j++, k++){
        if(text[j] != pattern[k]){
          break;
        }
        else if(k == m-1){
          s1 << "match at " << i - m + 2 << endl;
        }
      }
    }
  }
