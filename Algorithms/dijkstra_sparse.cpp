 ...
 * @ Modified time: 2021-05-23 00:19:57
...
void solve()
{
  ...
  vi parent(n, INF);
  distances[0] = 0;
  priority_queue<pii,vii,greater<pii>> q1;
  q1.push({distances[0], 0});
  while(!q1.empty()){
    auto [distance, vertex] = q1.top();
    q1.pop();
    ...
    tr(a, adj[vertex])
    {
      int to = a.first;
      int weight = a.second;
      if(distances[to] > distance+weight){
        distances[to] = distance + weight;
        parent[to] = vertex;
        q1.push({distances[to], to});
      }
    }
  }
