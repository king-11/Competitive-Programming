 ...
 * @ Modified time: 2021-05-23 00:39:33
...
void solve()
{
  ...
  rep(i,n){
    ...
    rep(j, n)
    {
      if(!visited[j] && p1.s > distances[j]){
        p1 = {j, distances[j]};
      }
    }
    int v = p1.f, distance = p1.s;
    visited[v] = 1;
    rep(j, n)
    {
      if(distances[j] > distance + adj[v][j]){
        distances[j] = distance + adj[v][j];
        parent[j] = v;
      }
    }
  }
